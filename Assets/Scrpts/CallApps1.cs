...
public class CallApps_Temp : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        exepath = Path.Combine(Application.dataPath, "../Apps/GAME" + currentNum + "/main." + type);
        readmePath = Path.Combine(Application.dataPath, "../Apps/GAME" + currentNum + "/readme.txt");
        if (Input.GetKeyDown(KeyCode.Z) && index == aaaaa && !exception)
        {
            LaunchApp();
        }
        if (Input.GetKeyDown(KeyCode.Z) && index == aaaaa && exception)
        ...
        if (Input.GetKeyDown(KeyCode.X) && index == aaaaa)
        {
            LaunchReadMe();
        }
        if (isNeedQuit)
